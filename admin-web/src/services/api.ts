...
class ApiService {
  ...
  async getSlots(): Promise<any[]> {
    const response = await this.api.get('/slots');
    return response.data;
  }
  ...
  async getAdvertisements(): Promise<Advertisement[]> {
    const response = await this.api.get('/advertisements');
    return response.data.data;
  }

  async createAdvertisement(data: Partial<Advertisement>): Promise<Advertisement> {
    const response = await this.api.post('/advertisements', data);
    return response.data.data;
  }

  async updateAdvertisement(id: number, data: Partial<Advertisement>): Promise<Advertisement> {
    const response = await this.api.patch(`/advertisements/${id}`, data);
    return response.data.data;
  }

  async deleteAdvertisement(id: number): Promise<void> {
    await this.api.delete(`/advertisements/${id}`);
  }

  async getAdSettings(): Promise<{ adsEnabled: boolean }> {
    const response = await this.api.get('/advertisements/settings');
    return response.data.data;
  }

  async updateAdSettings(adsEnabled: boolean): Promise<void> {
    await this.api.post('/advertisements/settings', { adsEnabled });
  }

  async uploadAdVideo(file: File): Promise<string> {
    const formData = new FormData();
    formData.append('file', file);
    const response = await this.api.post('/advertisements/upload', formData, {
      headers: {
  ...
  // ==========================================
  // WORKER LEAVES ENDPOINTS
  // ==========================================

  async getWorkerLeaves(status?: string) {
    const query = status ? `?status=${status}` : '';
    const response = await this.api.get(`/admin/worker-leaves${query}`);
    return response.data;
  }

  async updateWorkerLeaveStatus(id: string, newStatus: string) {
    const response = await this.api.patch(`/admin/worker-leaves/${id}/status`, {
      status: newStatus,
    });
    return response.data;
  }

  async assignBackupWorkerForLeave(id: string, backupWorkerId: number) {
    const response = await this.api.patch(`/admin/worker-leaves/${id}/assign-backup`, {
      backupWorkerId,
    });
    return response.data;
  }
