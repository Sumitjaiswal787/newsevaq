  ...
  private toRad(deg: number): number {
   ...
   private async assignWorkerForBooking(
    ...
    try {
      ...
      // Parse booking date and timeslot for checking slots
      ...
      // Filter workers who have available slots at this time
      const availableWorkersWithDistance: { worker: Worker; distance: number; slot: any }[] = [];

      for (const worker of workers) {
        let matchedSlot: any = null;
        if (startTimeDate && endTimeDate) {
          matchedSlot = await this.slotsService.findAvailableSlotFlexible(
            worker.id,
            startTimeDate,
            endTimeDate,
            true // exact/flexible matching only (disable same-day fallback)
          );
        }
      ...
      if (availableWorkersWithDistance.length === 0) {
        this.logger.warn(`No workers with available slots for service ${booking.serviceId} on ${booking.date} at ${booking.startTime}`);
        return { success: false, reason: 'No workers with available slots at this time' };
      }

      // Sort by distance (nearest first)
      availableWorkersWithDistance.sort((a, b) => a.distance - b.distance);

      // Log distances
      for (const { worker, distance } of availableWorkersWithDistance.slice(0, 5)) {
        this.logger.log(
          `Available worker ${worker.id} distance: ${distance.toFixed(2)}km`,
        );
      }

      // Assign the nearest available worker
      const nearestWorker = availableWorkersWithDistance[0];
      ...
      if (nearestWorker.slot) {
        booking.slotId = nearestWorker.slot.id;
        booking.slot = nearestWorker.slot;
        await this.slotsService.bookSlotAtomic(nearestWorker.slot.id);
        this.logger.log(`Locked slot ${nearestWorker.slot.id} for booking ${booking.id} under worker ${nearestWorker.worker.id}`);
      }
