...
export class NotificationsService {
  ...
  async notifyCustomerWorkerOnLeave(user: User, workerName: string, startDate: string, endDate: string): Promise<void> {
    if (!user.fcmToken) {
      console.log(`[notifyCustomerWorkerOnLeave] No FCM token for user ${user.id}`);
      return;
    }

    const notificationTitle = 'Worker on Leave';
    const notificationBody = `Your assigned worker ${workerName} will be on leave from ${startDate} to ${endDate}.`;
    
    await this.sendPushNotification(user.fcmToken, notificationTitle, notificationBody, {
      type: 'worker_on_leave',
    });
  }

  async notifyCustomerBackupAssigned(user: User, backupWorkerName: string, startDate: string, endDate: string): Promise<void> {
    if (!user.fcmToken) {
      console.log(`[notifyCustomerBackupAssigned] No FCM token for user ${user.id}`);
      return;
    }

    const notificationTitle = 'Backup Worker Assigned';
    const notificationBody = `A backup worker (${backupWorkerName}) has been assigned for your bookings from ${startDate} to ${endDate}.`;
    
    await this.sendPushNotification(user.fcmToken, notificationTitle, notificationBody, {
      type: 'backup_worker_assigned',
    });
  }
