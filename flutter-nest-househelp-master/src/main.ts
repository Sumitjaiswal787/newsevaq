  ...
  if (process.env.NODE_ENV !== 'production') {
   ...
   if (process.env.NODE_ENV === 'production' || process.env.RAILWAY_ENVIRONMENT_ID) {
     try {
       ...
       // Add OTP columns to booking
       try {
         await AppDataSource.query(`
           ALTER TABLE "booking" ADD COLUMN IF NOT EXISTS "otp" text;
           ALTER TABLE "booking" ADD COLUMN IF NOT EXISTS "isOtpVerified" boolean DEFAULT false;
         `);
         winstonLogger.log('info', 'Added otp and isOtpVerified columns to booking (if not exists)');
       } catch (e: any) {
         winstonLogger.log('warn', 'OTP columns may already exist: ' + e.message);
       }
