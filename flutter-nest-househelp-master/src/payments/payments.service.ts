  ...
  async createBookingAfterPayment(
    ...
    try {
      ...
      if (bookingData.id) {
        ...
        if (bookingData.assignmentId || bookingData.serviceRequestId) {
          ...
          if (String(srId).includes('-')) {
            serviceRequest = await this.dataSource.query(
              'SELECT id, date, "serviceId", "assignedWorkerId", "assignedSlotId", metadata FROM service_request WHERE publicId = $1 LIMIT 1',
              [srId]
            );
            this.logger.debug('Querying by publicId');
          } else {
            serviceRequest = await this.dataSource.query(
              'SELECT id, date, "serviceId", "assignedWorkerId", "assignedSlotId", metadata FROM service_request WHERE id = $1 LIMIT 1',
              [parseInt(srId)]
            );
            this.logger.debug('Querying by numeric ID');
          }
          this.logger.debug(`Service request query result: ${JSON.stringify(serviceRequest)}`);
          if (serviceRequest && serviceRequest.length > 0 && serviceRequest[0].date) {
            ...
            if (serviceRequest[0].assignedWorkerId) {
              ...
              this.logger.debug(`Got workerId from service request assignedWorkerId: ${validBookingData.workerId}`);
            } else if (serviceRequest[0].metadata) {
              let metadataObj = serviceRequest[0].metadata;
              ...
              if (metadataObj && metadataObj.preferredWorkerId) {
                validBookingData.workerId = metadataObj.preferredWorkerId;
                this.logger.debug(`Got workerId from metadata.preferredWorkerId: ${validBookingData.workerId}`);
              }
            }
            if (serviceRequest[0].assignedSlotId) {
              validBookingData.slotId = serviceRequest[0].assignedSlotId;
              this.logger.debug(`Got slotId from service request: ${validBookingData.slotId}`);
            }
          } else {
        ...
        // Lock the slot directly in this transaction to prevent race conditions
        if (validBookingData.slotId) {
          const slotRepo = queryRunner.manager.getRepository('Slot');
          ...
          if (!slot) {
            throw new Error(`Slot ${validBookingData.slotId} not found`);
          }
          if (slot.isBooked) {
            throw new Error(`Slot ${validBookingData.slotId} is already booked by another user`);
          }

          slot.isBooked = true;
          slot.currentBookings += 1;
          await slotRepo.save(slot);
          this.logger.log(`Locked slot ${validBookingData.slotId} successfully after payment`);
        }
