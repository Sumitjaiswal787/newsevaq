...
export class AdminService {
   ...
   */
  async updateWorker(id: number, updates: any): Promise<Worker | null> {
    ...
    if (updates.serviceIds) {
      const services = [];
      for (const serviceId of updates.serviceIds) {
    ...
    if (updates.bio !== undefined) worker.bio = updates.bio;
    if (updates.rating !== undefined) worker.rating = updates.rating;
    if (updates.reviewCount !== undefined) worker.reviewCount = parseInt(updates.reviewCount.toString(), 10);
    if (updates.yearsOfExperience !== undefined) worker.yearsOfExperience = parseInt(updates.yearsOfExperience.toString(), 10);
    if (updates.homesServedInArea !== undefined) worker.homesServedInArea = parseInt(updates.homesServedInArea.toString(), 10);
    if (updates.reliabilityStreak !== undefined) worker.reliabilityStreak = parseInt(updates.reliabilityStreak.toString(), 10);
    if (updates.isVerified !== undefined) worker.isVerified = updates.isVerified;
    if (updates.isTrained !== undefined) worker.isTrained = updates.isTrained;
    if (updates.isMonitored !== undefined) worker.isMonitored = updates.isMonitored;
    if (updates.isAvailable !== undefined) worker.isAvailable = updates.isAvailable;
    if (updates.serviceRadiusKm !== undefined) worker.serviceRadiusKm = parseFloat(updates.serviceRadiusKm.toString());
    if (updates.isActive !== undefined) worker.isActive = updates.isActive;
