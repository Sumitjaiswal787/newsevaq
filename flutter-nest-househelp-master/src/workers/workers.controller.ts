...
export class WorkersController {
  ...
  async getMyBookings(@Request() req: JwtRequest, @Query('status') status?: string) {
    ...
    const bookings = await this.workersService.getWorkerBookings(worker.id, status);
    return bookings.map(b => Object.assign({}, b, { isCompletionOtpRequired: !!b.completionOtp }));
  }
  ...
  async getMyAcceptedBookings(@Request() req: JwtRequest) {
    ...
    const bookings = await this.workersService.getAcceptedWorkerBookings(worker.id);
    return bookings.map(b => Object.assign({}, b, { isCompletionOtpRequired: !!b.completionOtp }));
  }
  ...
  async startBooking(
    ...
    try {
      ...
      return await this.workersService.startBooking(id, worker.id, otp);
    } catch (error: unknown) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      const errorStack = error instanceof Error ? error.stack : undefined;
  ...
  @UseGuards(JwtAuthGuard)
  async completeBooking(@Request() req: JwtRequest, @Param('id') id: string, @Body() body: { otp?: string }) {
    try {
      const worker = await this.workersService.findByUserId(req.user.userId);
      if (!worker) {
        throw new NotFoundException('Worker profile not found. Please complete your worker registration.');
      }
      return await this.workersService.completeBooking(id, worker.id, body.otp);
    } catch (error: unknown) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      const errorStack = error instanceof Error ? error.stack : undefined;
