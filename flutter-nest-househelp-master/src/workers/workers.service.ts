...
export class WorkersService {
  ...
  async getWorkerBookings(workerId: number, status?: string): Promise<Booking[]> {
    ...
    if (results.length > 0) {
      this.logger.debug(`[getWorkerBookings] First booking: ${JSON.stringify(results[0])}`);
    } else {
      this.logger.debug(`[getWorkerBookings] No bookings found for worker ${workerId} with status ${status}`);
    }
    return results;
  }
  ...
  async acceptBooking(bookingId: string, workerId: number): Promise<Booking> {
    const booking = await this.bookingsRepository.findOne({
      where: { id: bookingId },
      select: ['id', 'workerId', 'assignedWorkerId', 'status', 'location', 'userId', 'serviceId', 'date', 'startTime', 'endTime', 'amount', 'createdAt', 'updatedAt', 'startedAt']
    });
    if (!booking) {
   ...
   */
  async startBooking(bookingId: string, workerId: number, otp?: string): Promise<Booking> {
    const booking = await this.bookingsRepository.findOne({
      where: { id: bookingId },
      select: ['id', 'workerId', 'assignedWorkerId', 'status', 'location', 'userId', 'serviceId', 'date', 'startTime', 'endTime', 'amount', 'createdAt', 'updatedAt', 'otp', 'isOtpVerified', 'startedAt']
    });
    if (!booking) {
      throw new NotFoundException('Booking not found');
    }
    ...
    if (booking.otp && !booking.isOtpVerified) {
      if (!otp) {
        throw new BadRequestException('An OTP is required to start this job.');
      }
      if (booking.otp !== otp) {
   ...
   * Complete a job
   */
  async completeBooking(bookingId: string, workerId: number, otp?: string): Promise<Booking> {
    const booking = await this.bookingsRepository.findOne({
      where: { id: bookingId },
      select: ['id', 'workerId', 'assignedWorkerId', 'status', 'location', 'userId', 'serviceId', 'date', 
'startTime', 'endTime', 'amount', 'createdAt', 'updatedAt', 'startedAt', 'completionOtp', 'isCompletionOtpVerified']
    });
    if (!booking) {
      throw new NotFoundException('Booking not found');
    }
    ...
    if (booking.completionOtp && !booking.isCompletionOtpVerified) {
      if (!otp) {
        throw new BadRequestException('Completion OTP is required');
      }
  ...
  async getNearbyWorkersCount(lat: number, long: number, radius: number = 5): Promise<number> {
    const count = await this.workersRepository
      ...
      // Removing the isAvailable constraint so verified professionals are counted
      .getCount();
  ...
  async getWorkerStats(): Promise<{
    ...
    try {
        ...
        let avgResponseTime = Math.round(avgSeconds / 60) || 14; // Default to 14 minutes if no data
        if (avgResponseTime > 60) avgResponseTime = 14; // Cap unrealistic response times
