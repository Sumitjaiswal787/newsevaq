...
export class AppController {
  ...
  async testAssignment() {
    ...
    try {
      ...
      const reqStart = 10.0; // 10:00 AM
      const reqEnd = 12.0;   // 12:00 PM
      const serviceId = 1;

      // 1. Fetch worker Suvam (18)
      const worker = await ds.query(`
      ...
      // 2. Fetch Suvam's bookings on July 25
      const bookings = await ds.query(`
        SELECT id, "startTime", "endTime", "date", notes, status
        FROM booking
        WHERE "workerId" = 18 AND date = $1 AND status IN ('requested', 'confirmed', 'in_progress')
      `, [dateStr]);

      const overlapChecks = [];
      let overlaps = false;

      // Helper: parse time strings like "17:30:00" to fractional hours
      ...
      // 3. Fetch slots for Suvam on July 25
      const slots = await ds.query(`
        SELECT id, "startTime", "endTime", "isBooked"
        FROM slot
        WHERE "workerId" = 18 AND date = $1
