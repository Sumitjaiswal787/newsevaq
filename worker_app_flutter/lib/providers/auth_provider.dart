...
class AuthProvider extends ChangeNotifier {
  ...
  Future<bool> updateWorkerProfile(Map<String, dynamic> data) async {
    if (_worker == null) return false;
    _isLoading = true;
    _error = null;
    notifyListeners();
    ...
    try {
      final response = await _apiService.patch('workers/me', data);
      if (response != null) {
        await fetchWorkerProfile();
        return true;
      }
  ...
  Future<bool> updateWorkerServiceArea(double lat, double lng, String address) async {
    if (_worker == null) return false;
    _isLoading = true;
    _error = null;
    notifyListeners();
    ...
    try {
      final response = await _apiService.patch('workers/me/service-area', {
        'latitude': lat,
        'longitude': lng,
        'address': address,
        'radiusKm': 5.0,
